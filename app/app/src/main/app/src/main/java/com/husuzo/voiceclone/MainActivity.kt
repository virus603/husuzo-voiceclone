...
class MainActivity : AppCompatActivity() {
    ...
    // !!! Burada server ünvanını özün dəyiş: məsələn http://192.168.1.100:5000/
    private val serverBase = "http://YOUR_SERVER_IP:5000/"
    ...
    private fun checkPermissions() {
        val needed = arrayOf(Manifest.permission.RECORD_AUDIO, Manifest.permission.WRITE_EXTERNAL_STORAGE)
        val missing = needed.any { ContextCompat.checkSelfPermission(this, it) != PackageManager.PERMISSION_GRANTED }
        if (missing) {
            ActivityCompat.requestPermissions(this, needed, 123)
        }
    }
    ...
    private fun startRecording() {
        ...
        recorder = MediaRecorder().apply {
            ...
            try { prepare(); start() } catch (e: Exception) { e.printStackTrace(); tvStatus.text = "Record error: ${e.message}" }
        }
        isRecording = true
    ...
    private fun synthesizeText() {
        ...
        if (text.isEmpty()) { tvStatus.text = "Mətni daxil et"; return }
